...
| A3 | **基板温度センサー** | IN | `docs/S/safety_logic.md` §1 #3 | 過熱監視。**2026-08-06、10kΩ NTCサーミスタ＋分圧抵抗10kΩ（在庫の抵抗セットから充当）を採用決定。****素子は2026-08-06に調達済み（秋月 103AT-2-34119、ねじ止めタイプ）。実装・閾値設定は未着手** |
...
- **v1.3 2026-08-06** A3の備考欄を更新。素子（10kΩ NTCサーミスタ、秋月 103AT-2-34119 ねじ止めタイプ）を2026-08-06に調達済みとした（実装・閾値設定は未着手）。
