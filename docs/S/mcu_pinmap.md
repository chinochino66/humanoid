...
| A3 | **基板温度センサー** | IN | `docs/S/safety_logic.md` §1 #3 | 過熱監視。**2026-08-06、10kΩ NTCサーミスタ＋分圧抵抗10kΩ（在庫の抵抗セットから充当）を採用決定。**素子は未入手・未実装 |
...
- [ ] 基板温度センサーの実装位置と閾値（`docs/S/safety_logic.md` §3）（素子は10kΩ NTCサーミスタで決定済み。2026-08-06）
...
- **v1.1 2026-08-06** 第3層に在庫のArduino Mega 2560の2枚目を充てることを決定した旨を反映（`docs/decisions/2026-08-06_layer3_controller_selection.md`）。第3層Mega（安全用）のピン割当セクション（§3、信号一覧のみ・ピン番号は未定）を新設し、両Megaの共通GND必須の注記を追加した。第2層D10の備考へ第3層側受信ピンが未定である旨を追記した。
- **v1.2 2026-08-06** A3（基板温度センサー）の備考欄へ、10kΩ NTCサーミスタ＋分圧抵抗10kΩ（在庫の抵抗セットから充当）を採用決定した旨を追記した（素子は未入手・未実装）。§4未決事項の該当行へ素子確定済みである旨を追記した。
