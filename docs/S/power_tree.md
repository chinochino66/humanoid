...
#### プリチャージ実装方式の決定（2026-08-11）

**採用：2個の独立したスイッチによる2段階投入（案a相当）。追加調達なし。**

在庫の「12V 20A ミニロッカースイッチ ×5」から2個を使用する。

```
電池+ ── [SW1] ──┬── [100Ω/2W] ──┬── 12V母線
                  │                │
                  └─── [SW2] ──────┘
```

操作手順：
1. SW2 が OFF であることを確認する
2. **SW1 を ON** にする。100Ω 経由でコンデンサが充電される（約1.1秒待つ）
3. **SW2 を ON** にする。抵抗がバイパスされ通常運転に入る

遮断時は SW1 を OFF にし、SW2 は次回のため OFF へ戻す。

**却下した案（リレーによるバイパス）の理由：**
リレー方式は、コイル電源をどこから取るかという問題を生む。
12V母線から取るとプリチャージ完了前にリレーが動作する可能性があり、
これを避けるには独立したタイマー回路が必要となる。
第2層 Arduino は 12V母線の後段にあり電源投入時点では起動していないため、
マイコンによる制御も使えない。
結果として部品点数が増え、かつ電源投入シーケンスが能動回路の正常動作に依存することになる。
これは S-Phase 0 の設計思想（非常停止を通信バスと AI OS から独立させる）に反する。

**2スイッチ方式の利点：**
能動素子を一切使わず、投入シーケンスが人間の操作によって物理的に保証される。
また SW2 の投入を忘れた場合でも 100Ω 経由では 0.146A しか流れず
大電流負荷が動作しないため、異常に気づける。
リレー方式のタイマー故障は無音で進行する点で劣る。

**残存リスク（明記）：**
SW2 を ON にしたまま SW1 を投入すると、プリチャージを経ずに突入電流が流れる。
人間の操作順序に依存する方式であるため、このリスクは構造的に除去できない。
対策として、SW1 に「①」、SW2 に「②」のラベルを貼付し、
起動手順を機器に表示する。SW2 はカバー付きとするか操作しにくい位置へ配置する。

**将来の改善方針：**
より確実な方式として、OFF → PRECHARGE → RUN の3ポジションロータリースイッチにより
順序を物理的に強制する構成がある。DC20A 対応品は高価であるため L系学習機では採用しないが、
R系（商用）へ進む際に再評価する。

未実装。実装後、SW1 投入から母線電圧が定常値に達するまでの時間を実測すること。
...
**2026-08-06、PチャネルMOSFETによる逆接続保護回路を採用する（同日中に案1から変更）。**

採用部品：
- PchパワーMOSFET **2SJ334**（60V／30A、秋月電子 販売コード102846）
- ゲート保護用ツェナーダイオード（12V〜15V）×1
- ゲート抵抗 100kΩ（在庫の抵抗セットから充当）

構成：MOSFETのソースを12V母線の入力側、ドレインを負荷側に置き、ゲートをゲート抵抗経由でGNDへ接続する。順接続時はVgsが負となりMOSFETがONし、逆接続時はOFFとなって後段を保護する。

選定根拠：
- 耐圧60Vは母線12.8〜14.6Vに対し約4倍の余裕がある
- 定格30Aは定常1.13A、ストール時のピークに対しても十分な余裕がある
- 損失はオン抵抗×電流の二乗。Rds(on)を50mΩと仮定しても定常1.13Aで約0.064Wであり、却下したショットキー案（0.3〜0.6W）より1桁小さい

**当初採用した案1（理想ダイオードコントローラ LM74610-Q1等）を撤回した理由：**
LM74610-Q1はSOIC-8のSMDパッケージであり、専用基板の設計とチャージポンプ用コンデンサを要する。秋月電子に取扱いがなくDigiKey等からの取り寄せとなる。L系学習機の段階では実装難度と入手性が見合わない。PchMOSFET方式は損失面で理想ダイオードとほぼ同等でありながら、スルーホール部品3点で構成でき、空中配線でも実装できる。

**ゲート保護が必須である理由（重要）：**
本システムの12V母線にはTVSダイオード **SMBJ18CA** が実装されている。18V品のクランプ電圧は約29Vに達するため、サージ発生時にゲート-ソース間へ約29Vが印加される。PchMOSFETのゲート-ソース間絶対最大定格は±20V程度が一般的であり、これを超えるとゲート酸化膜が破壊される。したがってゲート-ソース間に12V〜15Vのツェナーダイオードを並列に入れ、ゲート抵抗100kΩでツェナー電流を制限する。

未実装。実装後に順方向電圧降下と発熱、および逆接続時の遮断を実測すること。実際のRds(on)はデータシートで確認し、本節へ追記すること。
...
- [ ] **基板温度センサーの調達**（`docs/S/mcu_pinmap.md` でMega A3に割当済みだが現物が部品表に無い。部品表の「防水接触温度センサー」はESP32味覚ノード用、「ANENG TH202」は放射温度計のため常設不可）→ 10kΩ NTCサーミスタで確定（2026-08-06）
- [x] ~~理想ダイオードコントローラ（LM74610-Q1等）＋NチャネルMOSFET 各1~~ → **方式変更により不要**（2026-08-06、PchMOSFET方式へ変更。§7-4参照）
- [x] **PchパワーMOSFET 2SJ334（60V30A、秋月 販売コード102846）** → **調達済み**（2026-08-06。12V母線逆接保護）
- [x] **ゲート保護用ツェナーダイオード（12V〜15V）** → **調達済み**（2026-08-06）
- [x] ~~10kΩ NTCサーミスタ ×1~~ → **調達済み**（2026-08-06、秋月 103AT-2-34119 ねじ止めタイプ）
- [x] ~~RXEF250 ×2以上（予備込み4本を推奨）~~ → **調達済み**（2026-08-06）
- [x] ~~HS-805BB ×2~~ → **調達済み**（2026-08-06）
- [x] ~~JX PDI-6221MG ×1~~ → **調達済み**（2026-08-06。180°版であるかは未確認）
- [x] ~~**M3×20 / M3×10 / 4mm×20**（i2Head組立。`docs/OPEN_ISSUES.md` #40）~~ → **調達済み**（2026-08-11。M3×20／M3×10／M4×20 皿小ねじ、ステンレス。M3ナットは在庫充足のため調達せず）

**ブリーダ抵抗（10kΩ ×2）は在庫の抵抗セットから充当するため買い足しリストに含めない。**

**2026-08-11時点で、L2〜L3 の実装に必要な部品はすべて調達済みまたは在庫充足である。**
プリチャージ用スイッチ（在庫ロッカースイッチ2個）、ブリーダ抵抗（在庫抵抗セットの10kΩ×2）は
追加調達を要しない。
...
- **v1.4 2026-08-06** §7-4で12V母線の逆接保護に案1（理想ダイオードコントローラ＋NチャネルMOSFET）を正式採用した（型番未確定・部品未入手・実装未着手）。§7-3にブリーダ抵抗の設計値（レールA・Bとも10kΩ/1/4W、時定数22ms、放電完了110ms）を追記した（未実装）。§11の買い足しリストへ理想ダイオードコントローラ＋MOSFET、10kΩ NTCサーミスタ、RXEF250、HS-805BB、JX PDI-6221MG、i2Head組立用ネジ類を追加し、基板温度センサーの調達行に10kΩ NTCサーミスタで確定した旨を追記、ブリーダ抵抗は在庫の抵抗セットから充当するため買い足しリストに含めない旨を注記した。
- **v1.5 2026-08-06** §7-4の採用方式を、同日中に案1（理想ダイオードコントローラ＋NチャネルMOSFET）からPチャネルMOSFET（2SJ334＋ゲート保護ツェナー＋ゲート抵抗100kΩ）方式へ変更した（未実装）。§11の買い足しリストを更新：理想ダイオードコントローラ＋MOSFETを方式変更により不要とし、2SJ334・ゲート保護用ツェナーダイオードを調達済みとして新規追加。10kΩ NTCサーミスタ・RXEF250・HS-805BB・JX PDI-6221MGを調達済み（2026-08-06）へ更新した（JX PDI-6221MGは180°版であるかは未確認のまま）。
- **v1.6 2026-08-11** §7-2末尾にプリチャージ実装方式の決定を追記した。在庫の12V 20Aミニロッカースイッチ2個による2段階投入方式（案a相当）を採用し、追加調達なし。リレーによるバイパス方式はコイル電源とタイマー回路を要し電源投入シーケンスが能動回路に依存するため却下した（`docs/OPEN_ISSUES.md` #8）。未実装。§11の買い足しリストのM3×20／M3×10／4mm×20の行を調達済みへ更新し（`docs/OPEN_ISSUES.md` #40）、L2〜L3実装に必要な部品がすべて調達済みまたは在庫充足である旨の注記を追加した。
