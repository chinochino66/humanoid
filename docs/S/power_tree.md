...
flowchart TB
    ...
    RA --> PCA1["PCA9685 #1 V+<br/>目・瞼 4ch(2026年実装)<br/>表情7ch→R系2027年<br/>味覚1ch→S系"]
...
- レールA（PCA9685 #1、目・瞼4ch＋表情7ch(R系)＋味覚1ch(S系)、いずれも電力経路は共通）: `DIST → CUT → P1 → BKA → RA → PCA1`
...
| 6V-A | **5.8V設定** | LM2596 #A | PCA#1 目・瞼4ch（2026年実装範囲。表情7ch・味覚1chは別系統） | PTC2.5A（RXEF250相当）／SMBJ7.0A／2200µF 10V | INA219 @0x44 | **安全系で遮断** |
...
| 負荷 | 定常 | ピーク | 備考 |
|---|---|---|---|
| レールA：目2軸＋瞼2軸（2026年実装範囲、MG90S×4） | 〜0.3〜0.5A | 〜1.4A（2軸同時ストール想定） | 皮膚アンカー荷重なし。表情7ch（R系）は2027年R5着手時に追加、現時点の電力予算に含めない |
...
**レールAの理論上限（4軸全て同時ストール）は、micro級ストール電流の目安0.7A@6V（`docs/archive/2026-08-03_servo_comparison_hphase.md` の代表値、MG90S固有の実測値ではない）を用いると 4 × 0.7A ≒ 2.8A となり、LM2596の実用2A枠を超える。** 目・瞼の4軸すべてが同時にストールする状況（機構干渉・異物挟み込み等）は稀だが、上表の「ピーク」は現実的な想定として2軸同時ストール（1.4A）を採用した。同時駆動数の制限（§0設計方針）をレールAにも適用し、「同時ストールは最大2軸まで」を前提とすることを明記し、実測で検証する（`docs/OPEN_ISSUES.md` へ登録）。
...
| 負荷 | 出力側電力 | 効率 | 電池側換算電力 |
|---|---|---|---|
| レールA（0.4A@5.8V、2026年実装範囲＝目2軸＋瞼2軸4chの定常帯0.3–0.5Aの中央値） | 5.8V×0.4A = 2.32W | 0.80 | 2.32W ÷ 0.80 = 2.90W |
...
電池側等価電流: 10.13W ÷ 12.8V ≒ **0.79A**（旧文書が誤って分母に使った1.6Aとは別物）

深放電回避のため使用可能容量を8割とする（旧文書と同じ前提）：76.8Wh × 0.8 = 61.44Wh

**稼働時間（定常） = 61.44Wh ÷ 10.13W ≒ 6.06時間 → 実用約6.1時間**

**(ii) 全軸同時ピークを継続した場合（下限の目安・非現実的な最悪値）**

| 負荷 | 出力側電力 | 効率 | 電池側換算電力 |
|---|---|---|---|
| レールA（現実的ピーク1.4A@5.8V、2軸同時ストール想定） | 8.12W | 0.80 | 10.15W |
| レールB（ストール3A@5.0V） | 15.00W | 0.90 | 16.67W |
| 5V-LOGIC | 1.50W | 0.90 | 1.67W |
| ポンプ（排液ポンプ稼働、0.5A@12.8V、MOSFET損失無視） | 6.40W | ≈1.0 | 6.40W |
| **合計** | | | **34.89W** |

**稼働時間（全軸ピーク継続、下限の目安） = 61.44Wh ÷ 34.89W ≒ 1.76時間**

ソフトウェアで同時駆動数を制限しているため(ii)は現実の運転では継続しないが、瞬間的な同時ピークがバッテリ・PTC・レギュレータの定格内に収まることの確認には使う。なおレールAの理論上限（4軸同時ストール2.8A）まで含めた場合は電池側15.9Wとなり、合計は約44.6Wに増え、下限は61.44Wh÷44.6W≒1.38時間まで下がる。これは§6.1で述べた通り低確率の想定であり、ソフトウェアの同時駆動数制限で回避する前提とする。

**新旧比較**：旧値「約3.7時間」は測定点の誤り（レールAピーク電流を電池側電流として使用）による偶然近い数値であり、方法として妥当ではなかった。加えて旧文書のレールA負荷（micro同時5軸、うちR系軸を含む）は2026年の実装範囲と一致していなかった。修正後は、レールAを2026年実装範囲（目2軸＋瞼2軸4ch）に正した上で、定常運転**約6.1時間**、全軸ピーク継続という非現実的な下限で**約1.8時間**。AIコンピュータ搭載後・効率実測後・レールA理論上限の扱い確定後に再計算すること。
...
| 項目 | 節 | 結論 |
|---|---|---|
| 稼働時間の再計算 | §6.2 | レールAを2026年実装範囲(4ch)へ修正した上で定常約6.1時間、全軸ピーク継続で下限約1.8時間（効率値は未実測につき暫定） |
...
いずれも未実装。`docs/OPEN_ISSUES.md`（12V母線の逆接保護、影響度：高）で継続管理する。
...
PTCの直列抵抗による電圧降下：
- レールA定常（0.4A、2026年実装範囲）: 0.05Ω × 0.4A = 0.02V／現実的ピーク（1.4A、2軸同時ストール想定）: 0.07V／理論上限（2.8A、4軸同時ストール）: 0.14V
...
(a) トリップ電流5.0AはレールAのLM2596実用電流2Aを大きく上回るため、PTCはレギュレータ自体を保護しない
