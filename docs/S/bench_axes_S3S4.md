...
- **v1.1 2026-08-05** 部品表の「高荷重5軸（上唇・頬×2・喉ゲート・味覚リフト）→MG92B×5」が案(a)を前提としている事実を§1・§7に反映。正式採否は未確認のまま（`docs/OPEN_ISSUES.md` #13参照）。

**頭部外ベンチ。頭部の表に混ぜない。** throatGateとtasteSensorLiftは2026年時点では電気的に頭部のPCA9685基板（PCA9685 #1 ch11、PCA9685 #2 ch3）に接続されるが、系統としてはS系（頭部外ベンチ機能）として扱い、`docs/L/channel_map_L.md`（L系）には含めない。
...
**在庫上の注意**：`docs/R/reserved_axes.md` により、現在庫のMG92B×5個は2027年R系向け（3軸分＋予備2個）と整理されている。上表の2軸（tasteSensorLift／throatGate）を合わせるとR系3軸＋S系2軸＝計5軸となり、**数量上はR系の予備2個をS系へ充てれば足りる。**「別途調達が必要」と断定していた記述は撤回する。**部品表の「高荷重5軸（上唇・頬×2・喉ゲート・味覚リフト）→MG92B×5」の記載は、案(a)を前提としている。2026-08-06に案(a)を正式採用した（`docs/OPEN_ISSUES.md` #13 解決済み）。**

- 案(a)（**採用・部品表の前提**） R系の予備2個をS系ベンチ軸（tasteSensorLift／throatGate）へ充てる。R系は予備なしとなる。
- 案(b)（不採用） S系ベンチ軸用に別途2個調達し、R系の予備2個は維持する案。部品表には未反映。
...
- 【2026-08-06 解決】案(a)採用。ただしS系2軸は湿式環境のため故障時は再発注する方針
- [ ] 喉ゲートの非通電時閉機構（ばね等）
- [ ] 防水温度センサーの型式確定
...
- **v1.2 2026-08-06** MG92B予備2個の配分方針を案(a)で正式採用（`docs/OPEN_ISSUES.md` #13 解決済み）。§1の「正式採否は未確認」の記述を採用済みの記述へ更新し、§7の未決事項の該当行を「【2026-08-06 解決】案(a)採用。ただしS系2軸は湿式環境のため故障時は再発注する方針」へ置き換えた。
- **v1.3 2026-08-06** `docs/L/channel_map_L2.md` のリネームに伴い、本書内の相互参照を `docs/L/channel_map_L.md` へ更新した（内容の変更はない）。
