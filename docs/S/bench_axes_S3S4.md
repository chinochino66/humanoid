...
**在庫上の注意**：`docs/R/reserved_axes.md` により、現在庫のMG92B×5個は2027年R系向け（3軸分＋予備2個）と整理されている。上表の2軸（tasteSensorLift／throatGate）を合わせるとR系3軸＋S系2軸＝計5軸となり、**数量上はR系の予備2個をS系へ充てれば足りる。**「別途調達が必要」と断定していた記述は撤回する。**部品表の「高荷重5軸（上唇・頬×2・喉ゲート・味覚リフト）→MG92B×5」の記載は、以下の案(a)を前提としている。**ただし正式採否は未確認であり、実装前に確認すること（§7未決事項、`docs/OPEN_ISSUES.md` #13参照）：

- 案(a)（**部品表の前提**） R系の予備2個をS系ベンチ軸（tasteSensorLift／throatGate）へ充てる。R系は予備なしとなる。
- 案(b) S系ベンチ軸用に別途2個調達し、R系の予備2個は維持する。部品表には未反映。
...
- [ ] **tasteSensorLift／throatGate用MG92Bの配分方針の正式採否確認。**部品表では案a（R系予備2個を充当）を前提としている。案b（別途2個調達）は部品表に未反映。§1、`docs/OPEN_ISSUES.md` #13参照
- [ ] 喉ゲートの非通電時閉機構（ばね等）
- [ ] 防水温度センサーの型式確定
