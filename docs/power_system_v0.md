...
- **E-STOP**: 12V母線の直後に配置し、サーボ系・ポンプ系を含む下流の全アクチュエータを遮断する。ロジック（Mega/ESP32）はPCからのUSB給電のため生存し、ログと状態機械は保持される
...
5. 異常時はE-STOP即押し。ソフトはD18で押下を検知して状態遷移をHALT
 - **待機状態の定義**: 一定時間動作要求が無ければ OE=HIGH としてサーボ保持電流を遮断する（省電力かつ発熱抑制）。復帰は次の動作要求時に OE=LOW → 目標角送信の順で行う。この待機状態は状態機械の正式な状態として実装すること
...
- [ ] 排液ポンプのみ緊急時にも動作させる例外の是非（液体を抜くことが安全に繋がる場面がある）。作るなら専用経路と根拠を明記のこと。v1判断
...
- [ ] 逆接保護方式（v1）
- [ ] AC/バッテリ切替回路（v1）
- [ ] 自律化: 機体内蔵コンピュータ（Raspberry Pi 5想定）を胴体へ搭載し、外部PC接続を廃止する。12Vレールから5V/5A級の専用降圧を分岐させる枠が必要。v1課題
- [ ] 上記に伴い、ロジック系のUSB給電（現在Mega/ESP32はPC由来）を内蔵機からの給電へ移行する
