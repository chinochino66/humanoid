...
- **v1.5 2026-08-06** v1.4のch1(neckUpDown)「サーボ未確定」の判断を撤回。NeckServoHolderV2.stlをBlenderで確認したところ同一形状のL字ブラケットが2個含まれており、スライサー表示の外形55.0×58.5×22.0mmは2個分の合計（1個あたり約55.0×28.0×22.0mm）であったため、厚み22mmはサーボ高さの制約にならないと判明した。InMoov公式 Hardware and BOM ページ（https://inmoov.fr/default-hardware-map/、取得日2026-08-06）の「2x Hitec HS805BB HEAD (up/down, right/left)」の記載により、ch0(neckRotate)・ch1(neckUpDown)とも**HS-805BBで確定**とし、備考を共通の文言へ統一した（`docs/OPEN_ISSUES.md` #39）。
