...
| 0x41（PCA9685 #2） | 0 | neckRotate | L | HS-805BB ×2（決定1により発注保留解除、未発注。`docs/L/servo_assignment_L.md` §2.3参照） | レールB（5.0V） | 1500µs（標準値、未校正） | 未実測 | 未実測 | 小角度から実測開始 | 単軸駆動ルール適用 | 未定 | 未定 | 未定（公式HS-805BB準拠） | 未実装（未発注） | 首と顎の同時駆動禁止 |
...
| 0x41（PCA9685 #2） | 2 | jaw | L | MG996R／DS3218MG（選定中。`docs/L/servo_assignment_L.md` §2.2参照） | レールB（5.0V） | 該当なし（閉位置を0°基準とする） | 閉＝0° | **試験15°／ハードコード上限20°** | 0〜20° | **20°をソフトウェアの絶対リミットとしてハードコードする** | 未定 | 未定 | 未定 | 未実装（選定中） | 首と同時駆動禁止。機械ストッパーとソフトウェア制限の両方を併用 |
...
- [ ] jaw候補（MG996R／DS3218MG）の最終決定（`docs/L/servo_assignment_L.md` §2.2）
