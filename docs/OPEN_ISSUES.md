...
| 41 | PTC（RXEF250相当）の紛失・再調達 | 中 | 部品表には「ポリスイッチ PTC-RXEF72 定格2.5A」として在庫があるとされていたが、2026-08-06に現物を確認したところ紛失しており在庫ゼロである。設計上、レールA・レールBの各降圧入力側に1本ずつ、計2本が必要（`docs/S/power_tree.md` §5・§7-5）。なお部品表の型番表記「RXEF72」は誤りで、72は保持電流ではなく耐圧72Vを指す。正しい型番は**RXEF250**（保持2.5A／トリップ5.0A／72VDC／初期抵抗0.05Ω）である。部品表側の型番表記の訂正も必要。 | RXEF250を2本以上（予備を含め4本を推奨）再調達し、現物の刻印で型番を確認する | 12V母線の実配線前 | 部品表、`docs/S/power_tree.md` §7-5 |
