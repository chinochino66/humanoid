...
| 3 | 12V母線の逆接保護 | 高 | 未解決。候補2案（理想ダイオードコントローラ／ショットキーダイオード）を提示済みだが未実装。2026-08-06、案1（理想ダイオードコントローラ＋NチャネルMOSFET）を正式採用した（`docs/S/power_tree.md` §7-4）。案2のショットキー方式は定常0.3〜0.6Wの常時発熱を頭部内に持ち込むため却下した。具体的な型番は未確定、部品は未入手、実装は未着手。 | いずれかの方式を選定し実装。損失・発熱を評価。入手性を確認して型番を確定し、実装後に順方向電圧降下と発熱を実測する | 12V母線の実配線前 | `docs/S/power_tree.md` |
